(function() {
  ...
  function pastePhoneNumber(event) {
    event.preventDefault();
    var text = event.clipboardData ? event.clipboardData.getData('text') : '';
    event.target.value = formatPhoneNumber(text);
  }
  ...
  function renderSlots() {
    ...
    list.innerHTML = slots.map(function(slot) {
      var location = slot.location || (getTrainer() && getTrainer().location) || 'Timpanogos Baseball';
      return '<button type="button" class="v2-train-slot' + (slot._key === state.selectedSlotId ? ' is-selected' : '') + '" data-slot-id="' + escapeHtml(slot._key) + '">' +
        ...
        '<small class="v2-train-slot-location">' + escapeHtml(location) + '</small>' +
      '</button>';
    }).join('');
  ...
  function showSuccess(request, trainer) {
    ...
    if (phone) {
      phone.textContent = coachPhone
        ? 'Please text ' + coachName + ' at ' + coachPhone + ' with 24 hours notice.'
        : 'Please text me with 24 hours notice.';
    }
  ...
  function wireEvents() {
    ...
    $('trainCustomerPhone').addEventListener('keydown', allowPhoneKey);
    $('trainCustomerPhone').addEventListener('input', formatPhoneInput);
    $('trainCustomerPhone').addEventListener('paste', pastePhoneNumber);
